...
export function buildDeduction(candidate, inventory) {
  const items = [];
  for (const need of candidate.ingredients.filter((ingredient) => !ingredient.pantry)) {
    ...
    const batches = [...inventoryItem.batches].sort((a, b) => Number(b.useSoon) - Number(a.useSoon) || String(a.expiresOn || "9999").localeCompare(String(b.expiresOn || "9999")));
    for (const batch of batches) {
      ...
      items.push({ key: `${inventoryItem.id}:${batch.id}`, ingredientId: need.id, itemId: inventoryItem.id, batchId: batch.id, name: need.name, amount: Math.round((amountBase / available.factor) * 100) / 100, unit: batch.unit, max: Number(batch.quantity), precision: batch.precision, useSoon: Boolean(batch.useSoon), skip: false });
    ...
    if (remaining > 0 && batches.some((batch) => batch.quantity === null)) {
      ...
      items.push({ key: `${inventoryItem.id}:${batch.id}`, ingredientId: need.id, itemId: inventoryItem.id, batchId: batch.id, name: need.name, amount: 0, unit: batch.unit, max: null, precision: "unknown", useSoon: Boolean(batch.useSoon), skip: true });
    }
  }
  return { recipeId: candidate.recipe.id, recipeTitle: candidate.recipe.title, diners: candidate.diners, createdAt: new Date().toISOString(), items };
}
...
export function applyDeduction(document, deduction) {
  ...
  next.inventory = next.inventory.map((item) => ({ ...item, batches: item.batches.filter((batch) => batch.quantity !== 0) })).filter((item) => item.batches.length > 0);
  return next;
}
